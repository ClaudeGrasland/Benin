---
title: "OCHA-ROWCA"
subtitle: "Source de données EE CIST"
date: "`r Sys.Date()`"
author: 
 - name: Claude Grasland
   affiliation: Université Paris Diderot, FR 2007 CIST, UMR 8504 Géographie-cités
image: "figures/ocha_rowca.png"    
logo: "figures/ocha_rowca.png"  
output:
  rzine::readrzine:
    highlight: kate
...
# Les données OCHA-ROWCA


Nous commençons par effectuer une requête `Benin`  sur le site de données HDX ce qui conduit à une liste de 25 sources de données provenant soit de OCHA-ROWCA, soit d'autres agences des Nations-Unies (FAO, WorldBank, WolrdPop, ...). Il s'agit donc d'un véritable hub de concenration des données. Celles-ci sont de plus généralement très récentes.  

```{r}
knitr::include_graphics("figures/HDX.png")
```
...
adm0<-st_read("admin/ben_admbnda_adm0_1m_salb_20190816.shp",quiet = TRUE)
adm1<-st_read("admin/ben_admbnda_adm1_1m_salb_20190816.shp",quiet = TRUE)
adm2<-st_read("admin/ben_admbnda_adm2_1m_salb_20190816.shp", quiet = TRUE)
...
loc<-st_read("settlement/ben_plp_NGA.shp")
par(mar=c(0,0,0,0))
plot(adm2$geometry,col="lightyellow", border = "gray20",lwd=0.5)
...
don<-read.csv("pop2019/ben_adm2_pop.csv", sep=",",header=T, fileEncoding = "UTF-8")
don2<-don[,c(11:43)]
